...
vim.list_extend(ensure_installed, {
  ...
  { -- Highlight, edit, and navigate code
    ...
    -- [[ Configure Treesitter ]] See `:help nvim-treesitter`inint
    opts = {
...
}, { -- <-- ESTA llave cierra la tabla de configuración de plugins
  ui = {
...
--------------------------------------------------
-- Theme selector with Telescope (persistent)
--------------------------------------------------
...
-- Utility: write last-used theme to disk
...
-- Utility: safely set colorscheme
local function set_colorscheme(name)
  local ok = pcall(vim.cmd, "colorscheme " .. name)
  if not ok then
    vim.notify("Could not load theme: " .. name, vim.log.levels.ERROR)
    return false
  end
  save_theme(name)
  return true
...
vim.keymap.set("n", "<leader>Ts", function()
  require("telescope.pickers")
    .new({}, {
      ...
      finder = require("telescope.finders").new_table({ results = builtin_themes }),
      ...
      attach_mappings = function(_, map)
        ...
        local function apply(prompt_bufnr)
          ...
          if selected then
            set_colorscheme(selected.value)
          end
        end
...
-- Auto-load last theme on startup
local f = io.open(theme_file, "r")
if f then
  local last_theme = f:read("*l")
  f:close()
  if last_theme and vim.tbl_contains(builtin_themes, last_theme) then
